...
a.forget_freerecall = a.nCorrect_freerecall_delayed - a.nCorrect_freerecall_2;
a.propremember_freerecall = a.nCorrect_freerecall_delayed./a.nCorrect_freerecall_2;
a.forget_recognise = a.propcorrect_recognise_delayed - a.propcorrect_recognise_2;
a = movevars(a,'forget_recognise','After','participantID');
a = movevars(a,'propremember_freerecall','After','participantID');
a = movevars(a,'forget_freerecall','After','participantID');
writetable(a,'result_alf.csv');




% Merge OMT, OIS2, TMT, DSST data
exptlist = {'OMTpavlovia','DSST','TMT'};
for j = 1:length(exptlist)
    Q = readtable(fullfile('Results',['result_' exptlist{j} '.csv']));

    t = readtable(fullfile('ParameterNameList',['parameterList_task_' exptlist{j} '.xlsx']));
    plist = t.plist;
    typelist = t.typelist;
    ...
    for i  = 1:length(plist)
        switch typelist{i}
            case 'num'
                a.(plist{i}) = nan(size(a.participantID));
            case 'time'
                a.(plist{i}) = NaT(size(a.participantID));
            case 'cell'
                a.(plist{i}) = cell(size(a.participantID));
        end
    end

    for s = 1:size(Q,1)
        idx = find(strcmp(a.participantID, Q.participantID{s}));
        if ~isempty(idx)
            for i = 1:length(plist)
                try
                    a.(plist{i})(idx) = Q.(plist{i})(s);
                catch
                    a.(plist{i}){idx} = Q.(plist{i}){s};
                end
            end
...
ois2 = readtable('result_ois2.csv');
...
writetable(a,'result_alf_OxCog.csv');
...
ylist = {'forget_ois2','DelayedCategoryAccuracy','ProportionCorrect_3item_4secs','Misbinding_Simple_3item_4secs',...
